...
public class HLDB {
  ...
  public class Table {
    ...
    func insertAndUpdate(insertRows: [Row], updateRows: [Row]) -> Future<DB.Result, NoError> {
      ...
      if updateRows.count > 0 {
        for row in updateRows {
          if let primaryKeyVal = row.fields[primaryKey] as? CustomStringConvertible {
            var pairs: [String] = []
            var args: [AnyObject] = []
            for (k, v) in row.fields {
              if let k = k as? CustomStringConvertible {
    ...
    public func upsert(rows: [Row]) -> Future<DB.Result, NoError> {
      ...
      for row in rows {
        if let rowId = row.fields[primaryKey] as? CustomStringConvertible {
          idList.append("\(rowId)")
      ...
      db.txBlock { fmdb in
        ...
        case .Items(let items):
          ...
          for item in items {
            if let v = item[self.primaryKey] as? CustomStringConvertible {
              foundIds["\(v)"] = true
            }
          }
        }
        ...
        if foundIds.count == 0 {
          ...
          for row in rows {
            if let rowId = row.fields[self.primaryKey] as? CustomStringConvertible {
